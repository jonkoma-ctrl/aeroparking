      ...
      <div className="container-main max-w-lg text-center">
        {isApproved && (
          <>
            ...
            <p className="mb-6 text-brand-500">
              Tu reserva fue confirmada. Te enviamos un email con los detalles y las instrucciones.
            </p>
          </>
        )}
        {isPending && (
        ...
        {isRejected && (
          <>
            ...
            <p className="mb-6 text-brand-500">
              No pudimos procesar tu pago. Podés intentar de nuevo.
            </p>
            <Link
              href="/reservar/puerto"
              className="inline-block rounded-lg bg-brand-900 px-6 py-2.5 text-sm font-semibold text-white hover:bg-brand-800"
            >
              Intentar de nuevo
            </Link>
          </>
        ...
        {(isApproved || isPending) && reservationId && (
          <Link
            href={`/mi-reserva?id=${reservationId}`}
            className="inline-block rounded-lg bg-brand-900 px-6 py-2.5 text-sm font-semibold text-white hover:bg-brand-800"
          >
            Ver mi reserva
          </Link>
